...
export function WhatsAppFloat() {
  ...
  return (
    <a
      href={`https://wa.me/919948729999?text=${encodeURIComponent("Hello Prasanth Roadlines, I need a transport quote.")}`}
      target="_blank"
      rel="noopener noreferrer"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className="fixed bottom-6 right-6 z-[9980] flex items-center gap-3 group"
    ...
    >
      ...
      <AnimatePresence>
        {hovered && (
          <motion.span
            ...
            className="bg-[#111] text-white text-xs font-semibold px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap pointer-events-none"
          >
            Chat on WhatsApp
          </motion.span>
        )}
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.95 }}
        className="relative h-14 w-14 rounded-full flex items-center justify-center shadow-lg"
